...
public class AppointmentAddController implements Initializable {
    ...
    @FXML private ComboBox combo_customer;
    ...
    ObservableList<Customer> customer_combo_list = FXCollections.observableArrayList();    // List to hold all available customers
    ...
    public void onAddAppointmentButtonClick(){
    ...
    // Used to populate the combo_customer combo box
    public void buildCustomerData(){
        ...
        try{
            ...
            while(results.next()) {  
                ...
                customer_combo_list.add(customer);
            }
            
            combo_customer.setItems(customer_combo_list);
    ...
    // LAMBDA EXPRESSION: Verifies the business hours of the user inputted day. I used this lambda because of its quick and simple way to retrieve the verification result
    // It is more suited in a lambda expression rather than its own method because of its quick one-line condition checking
    BooleanLambda verifyBusinessHours = (day) -> {
    ...
    public Boolean verifyOverlap(LocalDateTime input_time_from, LocalDateTime input_time_to){
        ...
        try{
            ...
            while(results.next()){
                LocalDateTime sql_from = LocalDateTime.parse(results.getString("start").replace(".0", ""), formatter);
                LocalDateTime sql_to = LocalDateTime.parse(results.getString("end").replace(".0", ""), formatter);
                
                // Convert SQL records to user's timezone & back to GMT <easier to compare>
                LocalDateTime from_converted = convertDate(convertToCurrentZone(sql_from));
                LocalDateTime to_converted = convertDate(convertToCurrentZone(sql_to));                
    ...
    public Boolean checkNotInPast(LocalDateTime input_time){
        if(input_time.isBefore(LocalDateTime.now(ZoneId.of("GMT")))){
    ...
    // Grabs all user inputted data and saves it to the database
    public void addAppointment(){
        Customer customer = (Customer) combo_customer.getSelectionModel().getSelectedItem();
        int customerID = customer.getId();  // Retrieve the ID of the selected customer;
        ...
        try {
            if(!verifyBusinessHours.compute(day)){   // Verify correct business hours
            ...
            LocalDateTime dt_from = day.atTime(hour_from, min_from);
            ...
            LocalDateTime dt_to = day.atTime(hour_to,  min_to);

            // Convert the dates to be usable in SQL
            LocalDateTime converted_from = convertDate(dt_from);
            LocalDateTime converted_to = convertDate(dt_to);
            ...
            try {
                connection = db.jdbc_connection.connect();
                String sql = "INSERT INTO appointment (customerID, userID, title, description, location, contact, type, url, start, end) VALUES (?,?,?,?,?,?,?,?,?,?)";
                ...
                pstmt.setString(9, converted_from.format(DateTimeFormatter.ofPattern("uuuu-MM-dd HH:mm:ss")));
                pstmt.setString(10, converted_to.format(DateTimeFormatter.ofPattern("uuuu-MM-dd HH:mm:ss")));
            ...
            lbl_dataError.setText("New appointment successfully added.");
            clearFields();  // Clear textfields
    ...
    public LocalDateTime convertDate(LocalDateTime time_input) {
        ...
        LocalDateTime converted_time = gmt_time.toLocalDateTime();  // Convert the GMT ZonedDateTime to a LocalDateTime to store in database
    ...
    public void buildCustomerCombo(){
        combo_customer.getSelectionModel().selectFirst(); // Select the first element
        
        // Update each customer to only show their name within the combo box
        ...
        combo_customer.setCellFactory(factory);
        combo_customer.setButtonCell(factory.call(null));  
    }
    
    // btn_cancel : event
    // Cancels the add appointment function and returns to main appointment view
    public void cancelAddAppointment(javafx.event.ActionEvent event) throws IOException {
    ...
    // Clears text fields after adding appointment
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        date_day.setValue(LocalDate.now()); // Setting date in calendar day to today
        
        buildCustomerData();    // Retrieve all customers and populate them into the combo box
        buildTimeWindowData();    // Retrieve static timewindows and inserts them into the combo box
       
        buildCustomerCombo();   // Builds and populates the customer combo box with data from local Observable list
...
// Exception class that handles business hour verification
class InvalidBusinessHoursException extends Exception{
    InvalidBusinessHoursException(String s){
        super(s);
    }
}

// Exception class that handles overlapping appointments for users
class OverlappingAppointmentsException extends Exception{
    OverlappingAppointmentsException(String s){
        super(s);
    }
}

interface BooleanLambda<T> {
    Boolean compute(LocalDate d);
}